...
public class NeuralNetworkPSO {
    ...
    public NeuralNetworkPSO(int inputSize, int[] hiddenLayerSizes, int outputSize, String activationType) {
...
class Particle {
    ...
    private double personalBestError = Double.MAX_VALUE;

    public Particle(List<double[][]> weights, List<double[]> biases) {
        Random rand = new Random();
        int positionSize = calculatePositionSize(weights, biases);
        position = new double[positionSize];
        velocity = new double[positionSize];
        personalBestPosition = new double[positionSize];
    ...
    public void updatePersonalBest(double error) {
        if (error < personalBestError) {
            personalBestError = error;
            personalBestPosition = position.clone();
        }
    }

    public void updateVelocity(double[] globalBestPosition, double inertiaWeight, double cognitiveCoefficient, double socialCoefficient) {
        Random rand = new Random();
        for (int i = 0; i < velocity.length; i++) {
            double cognitive = cognitiveCoefficient * rand.nextDouble() * (personalBestPosition[i] - position[i]);
            double social = socialCoefficient * rand.nextDouble() * (globalBestPosition[i] - position[i]);
            velocity[i] = inertiaWeight * velocity[i] + cognitive + social;
        }
    }
    ...
    public void applyPositionToWeights(List<double[][]> weights, List<double[]> biases) {
        int index = 0;
        for (double[][] layerWeights : weights) {
            for (int i = 0; i < layerWeights.length; i++) {
                for (int j = 0; j < layerWeights[i].length; j++) {
                    layerWeights[i][j] = position[index++];
                }
            }
        }
        for (double[] layerBiases : biases) {
            for (int i = 0; i < layerBiases.length; i++) {
                layerBiases[i] = position[index++];
            }
        }
    }

    private int calculatePositionSize(List<double[][]> weights, List<double[]> biases) {
        int size = 0;
        for (double[][] layerWeights : weights) {
            size += layerWeights.length * layerWeights[0].length;
        }
        for (double[] layerBiases : biases) {
            size += layerBiases.length;
        }
        return size;
    }
}
